...
This project attempts to predict seven of a molecule's physical and chemical properties given the molecular structure. It ises CatBoost tuned with Optuna and trained with RDKit 2D descriptors and Morgan fingerprints.

An attempt at adding Mordred 3D conformer descriptors was made early on the, but the geometry optemization step kept hitting a multiprocessing bug, so that part is cut out and the current notebook runs without it.

## Results

The numbers below come from a single end-to-end run of the notebook after the pipeline change described above (Mordred removal, Optuna trials cut to 15). Optuna is unseeded, so a subsequent rerun may land a few percent off these exact figures.
...
### 1. Data filtering
Inorganic compounds and salts are removed, keeping only carbon-containing organic molecules. This is done by filtering out "." in salts' SMILES strings, marking disconnected pieces in one structure. 

### 2. Feature engineering
Two feature sets are computed for each molecule and concatenated into one table:

- **RDKit 2D descriptors**: calculated from the SMILES string, covering electronic, topological, and physical properties of the molecule
- **Morgan fingerprints**: 2048-bit circular fingerprints (radius 2) that encode which small substructures surround each atom

3D conformer descriptors (via Mordred) were tried at an earlier stage of this project but dropped because of a multiprocessing bug in the geometry optimization step.

### 3. Preprocessing
`VarianceThreshold` drops any feature that has the same value across every molecule
`StandardScaler` rescales what's left so every feature sits on a comparable numeric range.

### 4. Hyperparameter tuning
Optuna runs 15 trials per target property, searching over the number of boosting iterations, the learning rate, tree depth, L2 regularization strength (a penalty that keeps the model from overfitting), and how much of the feature set each tree samples. Every trial is scored against a validation set.
...
**Multi-task learning could help.** The seven targets are physically correlated (boiling point and critical temperature both reflect intermolecular forces), so a single multi-output model could exploit those correlations instead of training seven separate models, making it much faster to train one model than seven.

**Trying a graph neural network baseline.** Message-passing networks such as Chemprop operate directly on the molecular graph instead of relying on hand-crafted descriptors, and they tend to outperform descriptor-based models, especially on sparse targets like flash point.

**Putting Mordred 3D Descriptors back in the pipeline.** By fixing the mutliprocessing bug, we may be able to get much more accurate features (albeit slower) of the molecules; this could significantly improve the model's performance.
